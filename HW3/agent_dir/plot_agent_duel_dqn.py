...
class DQN(nn.Module):
    ...
    def __init__(self, channels, num_actions):
        ...
        self.head1 = nn.Linear(512, num_actions)
    ...
    def forward(self, x):
        ...
        x1 = self.lrelu(self.fc1(x.view(x.size(0), -1)))
        x2 = self.lrelu(self.fc2(x.view(x.size(0), -1)))
        A = self.head1(x1)
        V = self.head2(x2)
        A_mean = torch.mean(A, 1)
...
class AgentDQN(Agent):
    ...
    def update(self):
        ...
        expectQa = rewards + self.GAMMA * torch.max(nextQ, 1)[0]
    ...
    def train(self):
        ...
        with open('rewards_duel_dqn.pkl', 'wb') as f:
